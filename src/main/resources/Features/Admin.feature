Feature: Admin Functionality in Orange HRM
  ...
  Scenario Outline: Add and delete user with dynamic count validation
    Given User login with user name "<USERNAME>" and password "<PASSWORD>" to Orange HRM
    ...
    And Admin set Employee Name "<employeeName>"
    And Admin click on Status Button
    And Admin click Enabled for Status
    And set User Name "<employeeUsername>"
    And set Password "<addPassword>"
    And set Confirm Password "<addPassword>"
    When save Results
    ...
    Examples:
      | USERNAME | PASSWORD | employeeName | employeeUsername | addPassword |
      | Admin    | admin123 | M            | MrAli            | Qa123456f   |
